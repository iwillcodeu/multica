...
SELECT i.id, i.workspace_id, i.title, i.description, i.status, i.priority, i.category,
...
WITH candidate AS (
    SELECT
        ...
        COALESCE(sqlc.narg('category')::text, i.category) AS next_category,
    ...
    SELECT
        candidate.*,
        ROW(
            title, description, status, priority, category, assignee_type, assignee_id,
            position, start_date, due_date, parent_issue_id, project_id, stage
        ) IS DISTINCT FROM ROW(
            next_title, next_description, next_status, next_priority, next_category,
...
UPDATE issue AS i SET
    ...
    category = changed.next_category,
